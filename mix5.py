...
if frame_width > 1000: # 1280x720 -> High-definition
    ...
    rect_offset = (15, -10)
thickness = 1
text_size, _ = cv2.getTextSize('Caution - {:02.1%}'.format(0.3),
...
def at(frame):
    ...
    global sleep_frames_counter 

    global beta
    global frame_count
    global blink_th
    global smooth_attentiveness
    global  frame_rate 
    global frame_width    
    global frame_height
    global   fontScale 
    ...
    global alpha 
    global out_path
    global yawns, yawn_status
    global average_attentivenss, landmarks
    ...
    #avg_att = []
   # avg_att.append({'attentiveness' : smooth_attentiveness,
    				#"dt": update_dt.strftime("%d/%m/%Y %H:%M:%S")})
  #  fb_db.child("student-wise-att").child("student1").update({'attentiveness': smooth_attentiveness})
   # fb_db.child("avg-attentiveness").child("student1").update({"dt": update_dt.strftime("%d/%m/%Y %H:%M:%S")})
    #fb_db.child("avg-attentiveness").child("student1").update({"attentiveness": smooth_attentiveness})
    ...
    attentive.append([attentiveness,smooth_attentiveness,average_attentivenss])
    ...
    print("Head direction",direction)
    print("Gaze direction", gaze_text)
...
#    cv2.addWeighted(overlay, alpha, frame, 1 - alpha,0, frame)
    ...
    d=[str(int(smooth_attentiveness)),"29","1","2","8"]
    df = pd.DataFrame([d])
    df.columns =["Score","time","green","yellow","red"]
    df.to_csv("data.csv", index= False)
    return {'attentiveness' : smooth_attentiveness,"dt": update_dt.strftime("%d/%m/%Y %H:%M:%S")}
    #fb_db.child("avg-attentiveness").child("student1").update({"dt": avg_att[::-1][0]['dt']})
    #fb_db.child("avg-attentiveness").child("student1").update({"attentiveness": avg_att[::-1][0]['attentiveness']})
    

    # if cv2.waitKey(1) == 27:
    #    df=pd.DataFrame(attentive)
